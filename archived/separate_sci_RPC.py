...
caliDir = '/home/belle2/zhai/calibration_outputdb/proc12_chunk2/'
...
bklm_sci_normal = numpy.zeros(1, dtype = int)
bklm_sci_dead = numpy.zeros(1, dtype = int)
bklm_sci_hot = numpy.zeros(1, dtype = int)

bklm_rpc_normal = numpy.zeros(1, dtype = int)
bklm_rpc_dead = numpy.zeros(1, dtype = int)
bklm_rpc_hot = numpy.zeros(1, dtype = int)
...
tree.Branch('bklm_sci_normal', bklm_sci_normal, 'bklm_sci_normal/I')
tree.Branch('bklm_sci_dead', bklm_sci_dead, 'bklm_sci_dead/I')
tree.Branch('bklm_sci_hot', bklm_sci_hot, 'bklm_sci_hot/I')
tree.Branch('bklm_rpc_normal', bklm_rpc_normal, 'bklm_rpc_normal/I')
tree.Branch('bklm_rpc_dead', bklm_rpc_dead, 'bklm_rpc_dead/I')
tree.Branch('bklm_rpc_hot', bklm_rpc_hot, 'bklm_rpc_hot/I')
...
for x in range(0,len(rev)):
    ...
    myIni = int(ini[x])
    myFin = int(fin[x])
outfile = ROOT.TFile('proc12_chunk2_validation.root', 'recreate')
tree.Write()
outfile.Close()
